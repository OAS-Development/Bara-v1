...
export function useShortcuts() {
  ...
  const shortcuts: Shortcut[] = [
    ...
    { keys: ['g', 'l'], description: 'Go to Life Dashboard', handler: () => router.push('/life') },
    
    // Actions
    { keys: ['a'], description: 'Quick add task', handler: () => {
      const event = new KeyboardEvent('keydown', { key: 'n', metaKey: true })
      window.dispatchEvent(event)
    }},
    { keys: ['d'], description: 'Mark done/undone', handler: () => {
    ...
    { keys: ['Escape'], description: 'Close modals/Clear selection', handler: () => {
      setShowHelp(false)
      selectTask(null)
    ...
    if (key === 'g') {
      ...
      const handleGPrefix = (e: KeyboardEvent) => {
        ...
        const gShortcut = shortcuts.find(s => 
          s.keys.length === 2 && s.keys[0] === 'g' && s.keys[1] === nextKey
        )
        if (gShortcut) {
          e.preventDefault()
          gShortcut.handler()
