...
export function MoodTracker({ entries, days = 30 }: MoodTrackerProps) {
  const moodData = useMemo(() => {
    ...
    const recentEntries = entries.filter(entry => 
      new Date(entry.entry_date) >= cutoffDate && entry.mood
    );
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          {moodData.filter(m => m.count > 0).map(mood => (
            <div key={mood.mood} className="flex items-center justify-between p-2 rounded-lg bg-muted">
              <span className="flex items-center gap-2">
