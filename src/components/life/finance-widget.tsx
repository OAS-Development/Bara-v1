...
export function FinanceWidget() {
  ...
  const { accounts, transactions, budgets, getTotalBalance, getCategorySpending } =
    useFinanceStore()
  ...
  const totalSpent = budgets.reduce(
    (sum, b) => sum + getCategorySpending(b.category, b.period === 'monthly' ? 'month' : 'year'),
    0
  )
    ...
    <Card className="hover:shadow-lg transition-shadow">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={() => router.push('/life/finance')}>
            View All →
          </Button>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div>
          ...
          <p className={cn('text-3xl font-bold', totalBalance < 0 && 'text-red-500')}>
