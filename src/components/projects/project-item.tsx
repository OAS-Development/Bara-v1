...
import { ChevronRight, ChevronDown, Folder, FolderOpen, Archive, CheckCircle, Circle, MoreVertical } from 'lucide-react'
...
export function ProjectItem({ project, level = 0, onSelect, selectedId }: ProjectItemProps) {
  ...
  const getProjectIcon = () => {
    ...
    if (project.status === 'active' && hasChildren) return isExpanded ? <FolderOpen className="h-4 w-4 text-blue-500" /> : <Folder className="h-4 w-4 text-blue-500" />
  ...
  const getProjectTypeIndicator = () => {
    ...
    return <span className="text-xs text-gray-400 ml-1">{types[project.type as keyof typeof types]}</span>
  }
