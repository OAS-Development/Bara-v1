...
export class TimeRules {
  ...
  constructor(customWindows?: TimeWindow[]) {
    this.windows = customWindows || DEFAULT_TIME_WINDOWS
  }
  ...
  isBusinessHours(date: Date = new Date()): boolean {
    ...
    return businessHoursWindow ? this.isInWindow(date, businessHoursWindow) : false
  }
  ...
  getCurrentContext(date: Date = new Date()): TimeContext {
    ...
    const businessHoursWindow = this.windows.find((w) => w.id === 'business-hours')
    const isBusinessHours = businessHoursWindow ? this.isInWindow(date, businessHoursWindow) : false
